...
#handler -> quando qualcuno scrive al tuo bot cosa devi fare?
   disp.add_handler(CommandHandler("start", welcome))
   disp.add_handler(MessageHandler(Filters.regex('^.*$'), process_chat)) #cosa fa bot quando qualcun0 scrive file di testo
   disp.add_handler(MessageHandler(Filters.location, process_location)) #quando qualcuno condivide posizione con bot stesso
   disp.add_handler(MessageHandler(Filters.photo, photo_handler)) #quando manda immagine a bot
